...
function ParseDataSource(argDataSource, argVmGallery)
{
	...
	else if (argDataSource.indexOf(' 倍可亲</title>') > 0)
  {
	...
	else if (argDataSource.indexOf('http://www.ellechina.com') >= 0)
  {
	  return parseElleChina(argDataSource, argVmGallery);
  }
...
function parseElleChina(argDataSource, argVmGallery) 
{
  $("#divSourceName").html("parseElleChina");
	
  var today = new Date();
	// var author = GetMetaContent(argDataSource, "author");   alert("meta author:" + author);
	argVmGallery.galleryName = GetMetaProperty(argDataSource, "og:title"); // alert("argVmGallery.galleryName=" + argVmGallery.galleryName);
	argVmGallery.galleryDesc =  GetMetaContent(argDataSource, "description");;
  // alert("argVmGallery.galleryDesc: " + argVmGallery.galleryDesc);	

  argDataSource = getSubContent(argDataSource, '<!-- 组图——缩略图kv -->', false, '<a id="div_scrollup"', false);
	// <!-- 文章标题 -->

	argDataSource = argDataSource.replace(/\n/gi, " ");
  argDataSource = argDataSource.replace(/<img /gi, "\n<img ");
	argDataSource = argDataSource.replace(/<li class="" .*?>/gi, "");
	argDataSource = argDataSource.replace(/<\/li>/gi, "");
	argDataSource = argDataSource.replace(/<span.*?>/gi, "");
	argDataSource = argDataSource.replace(/<\/span>/gi, "");
	argDataSource = argDataSource.replace(/<p.*?>/gi, "");
	argDataSource = argDataSource.replace(/<\/p>/gi, "");
	argDataSource = argDataSource.replace(/^\s+/gi, "");
	argDataSource = argDataSource.replace(/	/gi, " ");	// tab
	argDataSource = argDataSource.replace(/\t/gi, " ");
	argDataSource = argDataSource.replace(/  +/gi, " ");
	argDataSource = argDataSource.replace(/ src=".*?.jpg"/gi, "");
	argDataSource = argDataSource.replace(/ ls_src=/gi, " src=");
	argDataSource = argDataSource.replace(/thumb_.*?_thumb_elle_/gi, "");
	argDataSource = argDataSource.replace(/thumb_.*?_thumb_.*?__/gi, "");
	// alert("argDataSource\n" + argDataSource); OK
	argDataSource = argDataSource.replace(/ alt="(.*?)".*?>/gi, ">\n$1\n");
	// alert("argDataSource\n" + argDataSource); // OK
	// argDataSource = argDataSource.replace(/<p style='text-align:left'>/gi, "");
	// argDataSource = argDataSource.replace(/<img .*?src=(.*?)\s.*?>/gi, "\n$1\n");

  // argVmGallery.imageList = getImageArray_ElleChina(argDataSource);
	argVmGallery.imageList = getGeneralImageArray(argDataSource);
}	// parseElleChina()

//*---------------------------------------------------------------------------------------*//

function getImageArray_ElleChina(arg9YaoMainContent) {
  if (arg9YaoMainContent == "") {
    alert("getImageArray_ElleChina(): arg9YaoMainContent is empty  ");
    return;
  }
  ...
  $("#divLog").append("<li>getImageArray_ElleChina(): sectionArray is an array: length=" + sectionArray.length + "</li>");
  ...
  $.each(sectionArray, function (i, section) {
	...
	//	imageArray.push(footerImage);
  return imageArray;
} // function getImageArray_ElleChina(argImageList)
...
function getGeneralImageArray(argDataSource) {
  if (argDataSource == "") { 
	...
	// alert("getGeneralImageArray() argDataSource:\n" + argDataSource);
	argDataSource = argDataSource.replace(/<img /gi, "\n<img ");
	// alert("getGeneralImageArray() argDataSource:\n" + argDataSource);
  ...
  $.each(lineArray, function (lineNo, line) 
    ...
    var urls = line.match(/(http.*?\.(jpg|jpeg|gif|png))/i);	
...
function GetMetaContent(argString, argMetaName)
{
  // <meta name="description" content="...................">
  var pattern = new RegExp('<meta name="' + argMetaName + '" content="(.*?)">');
...
function GetMetaProperty(argString, argMetaProperty)
{
  var pattern = new RegExp('<meta property="' + argMetaProperty + '" content="(.*?)">');
  var matchedContent = argString.match(pattern);
  if (matchedContent == null)
    return "";
  return matchedContent[1];
...
function getSubContent(argString, argBeginStr, argKeepBegin, argEndStr, argKeepEnd)
{
  var posBegin = argString.indexOf(argBeginStr);
    ...
    argString = argString.substring(posBegin);
  }
    ...
    argString = argString.substring(0, posEnd - 1);
  }

	// alert("argString:" + argBeginStr + "@" + posBegin + " | " + argEndStr + "@" + posEnd + " ~~ " + length + "\n" + argString);
  return argString;
}	//getSubContent()
